...
app.get("/status", (_q, res) => { res.set("Cache-Control","no-store"); res.json(state); });
app.get("/api/status", (_q, res) => { res.set("Cache-Control","no-store"); res.json(state); });
...
app.listen(PORT,"0.0.0.0",()=>{
  ...
  // La UI debe quedar disponible antes de comenzar el análisis completo.
  setTimeout(()=>{
    scan().catch(e=>logEvent("START_SCAN_ERROR",null,{message:e.message})).finally(function loop(){
