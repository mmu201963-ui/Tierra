...
function closePositionAtPrice(p, price, reason = "MANUAL") {
  ...
  if (state.closingSymbols[p.symbol]) return null;
  state.closingSymbols[p.symbol] = true;
  ...
  delete state.closingSymbols[p.symbol];
...
async function watchOpenPositions() {
  if (state.positionWatcherRunning) return;
  const positions = Object.values(state.positions);
  if (!positions.length) return;
  state.positionWatcherRunning = true;
  try {
    const marks = await Promise.all(positions.map(async p => {
      try { return { p, price: await liveMarkPrice(p.symbol) }; }
      catch (e) { return { p, error: e.message }; }
    }));
    for (const m of marks) {
      const p = state.positions[m.p.symbol];
      if (!p || !Number.isFinite(m.price)) continue;
      p.markPrice = m.price;
      const age = Date.now() - p.openedAt;
      let reason = null;
      if (p.side === "LONG") {
        if (m.price <= p.stop) reason = "STOP_FAST";
        else if (m.price >= p.tp) reason = "TP_FAST";
      } else {
        if (m.price >= p.stop) reason = "STOP_FAST";
        else if (m.price <= p.tp) reason = "TP_FAST";
      }
      if (!reason && age >= CFG.maxHoldMs) reason = "TIME_FAST";
      if (reason) closePositionAtPrice(p, m.price, reason);
    }
    const live = Object.values(state.positions);
    state.floatingPnl = live.reduce((sum, p) => {
      if (!Number.isFinite(p.markPrice)) return sum;
      const raw = p.side === "LONG" ? (p.markPrice - p.entry) * p.qty : (p.entry - p.markPrice) * p.qty;
      const fees = p.allocation * CFG.feeRate + (p.allocation + raw) * CFG.feeRate;
      return sum + raw - fees;
    }, 0);
    state.equity = state.cash + live.reduce((sum, p) => sum + p.allocation, 0) + state.floatingPnl;
  } finally {
    state.positionWatcherRunning = false;
  }
}
...
app.listen(PORT,"0.0.0.0",()=>{
  ...
  // The HTTP server becomes available BEFORE any market scan starts.
  // This prevents the first browser request from competing with the initial scan.
  setTimeout(()=>{
    scan().catch(e=>logEvent("START_SCAN_ERROR",null,{message:e.message}));
  },1000);
  setInterval(()=>{
    scan().catch(e=>logEvent("SCAN_ERROR",null,{message:e.message}));
  },CFG.pollMs);
  // Position protection is independent of the full-market scan.
  setInterval(()=>{
    watchOpenPositions().catch(e=>logEvent("POSITION_WATCH_ERROR",null,{message:e.message}));
  },2000);
});
